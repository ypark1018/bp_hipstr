...
#allSTR - looks at all STR score total length of genotype
#del - looks at deleterious regions score total length of genotype
...
def main():
    if len(sys.argv) < 4:
        sys.exit("USAGE: python variant_score.py [input vep vcf] [deleterious score threshold] [output prefix]")
    DEL_THRESHOLD = int(sys.argv[2])
    vep = []
    if ".gz" in sys.argv[1]:
        import gzip
    ...
    annot_output = sys.argv[3] + ".annot" #output file name
    f_output = open(annot_output, 'w+')
    f_output.truncate()
    f_output.write("#CHR POS ALLELE IMPACT VARIANT GENE\n")
    
    allSTR_score_output = sys.argv[3] + ".allSTR.sum"
    g_output = open(allSTR_score_output, 'w+')
    g_output.truncate()
    
    del_score_output = sys.argv[3] + ".del.sum"
...
#    for n,score in enumerate(STR_loci):
#        allSTR_sum_scores[n] = allSTR_sum_scores[n]/score
#        del_sum_scores[n] = del_sum_scores[n]/score
