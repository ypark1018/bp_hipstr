...
#fix so it outputs one variant per line..
import sys
...
def main():
    ...
    for locus in vep:
        #l_output = [locus[0], locus[1]]#l_output = [] #list holds output per locus
        ...
        for gene in genelist.keys():
            t_output = [locus[0], locus[1]]
            t_output.extend((genelist[gene][0], genelist[gene][1], genelist[gene][2], gene))
            f_output.write(" ".join(t_output)+ '\n')
